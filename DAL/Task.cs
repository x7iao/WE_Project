...
namespace WE_Project.DAL
{
    public class Task
    {
        public static void SendTask(Model.Member from, Model.Member to, string ttype, string msg, int replyID)
        {
        ...
        /// 得到一个对象实体
        /// </summary>
        public static Model.Task GetModel(object Id)
        {
        ...
        public static Model.Task GetReplyTask(int id)
        {
            List<Model.Task> TaskList = new List<Model.Task>();
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select * ");
            strSql.Append(" FROM Task where replyid = " + id + " ");
            DataTable table = DbHelperSQL.Query(strSql.ToString()).Tables[0];
            for (int i = 0; i < table.Rows.Count; i++)
            {
                TaskList.Add(TranEntity(table.Rows[i]));
