 ...
 inner join MemberConfig b on a.MID=b.MID  where  MState='1' and IsClose<>'1' and PPState in (0,1,2) and  t1.SQMoney <>  t1.MatchMoney order by SQDate desc");
        ...
        public static string Matching4(string offid, string getid)
        {
            lock (thisLock)
            {
                ...
                offerlist = BLL.MOfferHelp.GetListJoinMoMcMo("PPState in (0,1,2)  and mo.SQDate <= '" + now.ToString("yyyy-MM-dd HH:mm:ss") + "' and mo.Id in(" + offid + ")   and m.MState='1' and IsClose<>'1' and (mo.SQCode in(select TOP(1) mo2.SQCode from dbo.MOfferHelp mo2 where mo2.PPState in(0,1,2,3) and mo2.sqmid = m.MID ORDER BY mo2.PPState desc) or mo.SQCode in(select mo3.SQCode from dbo.MOfferHelp mo3 where mo3.PPState in(0, 1, 2) and mo3.sqmid in(select m3.mid from member m3 where m3.zdstatus = 0))) order by mo.SQDate,mc.EPXingCount desc");
                //管理员记录
        ...
        public static string Matching3(string offmid, string getmid)
        {
            lock (thisLock)
            {
                ...
                offerlist = BLL.MOfferHelp.GetListJoinMoMcMo("PPState in (0,1,2)  and mo.SQDate <= '" + now.ToString("yyyy-MM-dd HH:mm:ss") + "' and m.mid = ('" + offmid + "')   and m.MState='1' and IsClose<>'1' and (mo.SQCode in(select TOP(1) mo2.SQCode from dbo.MOfferHelp mo2 where mo2.PPState in(0,1,2,3) and mo2.sqmid = m.MID ORDER BY mo2.PPState desc) or mo.SQCode in(select mo3.SQCode from dbo.MOfferHelp mo3 where mo3.PPState in(0, 1, 2) and mo3.sqmid in(select m3.mid from member m3 where m3.zdstatus = 0))) order by mo.SQDate,mc.EPXingCount desc");
                //管理员记录
                List<Model.MGetHelp> getadminlist = new List<Model.MGetHelp>();
                //getadminlist = BLL.MGetHelp.GetListJoinMember("t1.PPState in (0,2) and SQMID in (select MID from Member where MState='1' and IsClose<>'1' ) and t2.PPLeavel is not NULL and PPLeavel>0  order by t1.SQDate asc");
                //会员记录
        ...
        public static string Matching2()
        {
            lock (thisLock)
            {
                ...
                offerlist = BLL.MOfferHelp.GetListJoinMoMcMo("PPState in (0,1,2)  and mo.SQDate <= '" + now.ToString("yyyy-MM-dd HH:mm:ss") + "' and m.MState='1' and IsClose<>'1' and (mo.SQCode in(select TOP(1) mo2.SQCode from dbo.MOfferHelp mo2 where mo2.PPState in(0,1,2,3) and mo2.sqmid = m.MID ORDER BY mo2.PPState desc) or mo.SQCode in(select mo3.SQCode from dbo.MOfferHelp mo3 where mo3.PPState in(0, 1, 2) and mo3.sqmid in(select m3.mid from member m3 where m3.zdstatus = 0))) order by mo.SQDate,mc.EPXingCount desc");
                //管理员记录
                List<Model.MGetHelp> getadminlist = new List<Model.MGetHelp>();
