    ...
    public class RolePowers : BaseHandler
    {
        public override void ProcessRequest(HttpContext context)
        {
            ...
            if (!string.IsNullOrEmpty(iFVerify))
            {
                ...
                ListContents = BLL.Roles.RolsList[mType].PowersList.Where(emp => emp.Content.CState && emp.IFVerify == v).ToList();
            }
            else
                ListContents = BLL.Roles.RolsList[mType].PowersList.Where(emp => emp.Content.CState).ToList();
