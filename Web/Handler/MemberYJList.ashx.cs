    ...
    public class MemberYJList : BaseHandler
    {
        ...
        public override void ProcessRequest(HttpContext context)
        {
            ...
            for (int i = 0; i < ListMember.Count; i++)
            {
                ...
                sb.Append((i + 1) + (pageIndex - 1) * pageSize + "~");
                sb.Append("<a href=\"javascript:void(0);\" onclick=\"callhtml('ChangeMoney/JJJLList.aspx?id=" + ListMember[i].MID + "','奖金统计');\">" + ListMember[i].MID + "</a>~");
                //sb.Append(ListMember[i].MName + "~");
                sb.Append(ListMember[i].MAgencyType.MAgencyName + "~");
                foreach (string item in typeList)
                {
                    sb.Append(JJInfo[item + "Money"] + "~");
                    sumCount += Convert.ToInt32(JJInfo[item + "Count"]);
                    sumMoney += JJInfo[item + "Money"];
                }
                sb.Append("<span style='color:Red;font-weight:bold;'>" + sumMoney + "</span>");
                //sb.Append("<span style='color:Red;font-weight:bold;'>" + JJInfo["TakeOffMoney"] + "</span>" + "~");
                //sb.Append("<span style='color:Red;font-weight:bold;'>" + (sumMoney - JJInfo["TakeOffMoney"]) + "</span>");
                sb.Append("≌");
            }
