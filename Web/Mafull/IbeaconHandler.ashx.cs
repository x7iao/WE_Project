            ...
            try
            {
                ...
                if (file != null)
                {
                    ...
                    if (!ext.Equals(".gif") && !ext.Equals(".jpg") && !ext.Equals(".png") && !ext.Equals(".bmp") && !ext.Equals(".jpeg"))
                    {
                        ...
                        context.Response.Write("<script>window.parent.uploadSuccess('你上传的文件格式不正确！上传格式有(.gif、.jpg、.png、.bmp、.jpeg)');</script>");
                    ...
                    {
                        //这里window.parent.uploadSuccess()是我在前端页面中写好的javascript function,此方法主要用于输出异常和上传成功后的图片地址 
                        context.Response.Write("<script>window.parent.uploadSuccess('你上传的文件不能大于8MB!请重新上传！');</script>");
                        context.Response.End();
                    }
